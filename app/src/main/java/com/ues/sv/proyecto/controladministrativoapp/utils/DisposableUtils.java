...
public class DisposableUtils {
    ...
    public static void addComposite(CompositeCompletableCallback compositeInterface) {
        mDisposable.add(compositeInterface.completableAction()
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(compositeInterface::onCallback
                        , throwable -> {
    ...
    public static void addComposite(CompositeFlowableCallback compositeInterface) {
        mDisposable.add(
                compositeInterface.completableCallBack(
                        compositeInterface.flowableAction()
                                .subscribeOn(Schedulers.io())
                                .observeOn(AndroidSchedulers.mainThread())
                                .doOnError(throwable -> Log.e("ADD_FLOWABLEs_COMPOSITE", "Error ejecutar flowable", throwable))
                )
        );
    }
